...
Xs5 是一个面向 Debian 12 VPS 的公开出口聚合与 SOCKS5 管理工具。它按国家维护一个或多个固定 SOCKS5 地址，后台从 VPN Gate / Proxio / ProxyScrape 获取候选节点，执行真实出网验证、健康检查和故障切换；上层程序长期使用固定的 `服务器IP:端口`，后端出口发生变化时不需要重新修改 S5 配置。
...
- 支持 VPN Gate、Proxio、ProxyScrape 和全部来源候选策略；默认仍以 VPN Gate 为首选。
